...
- **Evidence:** Live www.sparkshark.com inspection 2026-05-11 found three tag IDs on the WP install: `GTM-W7V4RS7C` (Coalition-era GTM container — Brock does NOT own; **revised 2026-05-11 evening:** "loaded but unowned" is more accurate than "abandoned" — see SOURCE-OF-TRUTH.md §11), `AW-17076116496` (Google Ads conversion account with label `Hf8UCO6r84cZEN7Iyq0p`), and `GT-NGS794C2` (unified Google Tag via Site Kit — kept). Brock chose option 3: consolidate on the unified Google Tag and skip GTM as a management plane. Form submission test logged with Resend message ID `5cb36ea7-0b7e-4aae-8dc2-d4da078374b5`.
- **Analytics ownership picture (2026-05-11 evening, corrected after API-level reclaim):** the unowned `GTM-W7V4RS7C` container's payload sets GA4 measurement ID `G-QK02QH3SWY`. **Initial framing was wrong** — that measurement ID actually IS in a Brock-owned property: `488680346` "Spark Shark" under the Flanco Electric GA4 account (`347644522`). The UI add for the SA was blocked by a "doesn't match Google Account" validation bug; bypassed via `accessBindings.create` at v1alpha. 229 referral rows since 2025-05-09 extracted; see `ga4-referrals-property-488680346-all-history.csv`. **Only the GTM container layer is unowned, not the GA4 destination.** The `build.py:153` GTM→gtag.js swap below still retires the unowned `GTM-W7V4RS7C` from the live site (good) and should keep pointing at the same Brock-owned `G-QK02QH3SWY` destination (verify before shipping). Full corrected picture in `SOURCE-OF-TRUTH.md` §11.
...
7. **Move GA4 property `488680346` off the Flanco Electric account.** Defends Risk 6 + brand hygiene. Per Brock decision 2026-05-11: the live property must NOT remain under the legacy Flanco entity in the GA4 account hierarchy. The live measurement ID `G-QK02QH3SWY` lives in property `488680346` (currently in account `347644522` "Flanco Electric") — confirmed via dataStreams API this session, after pulling 229 referral rows from it. **Action:** GA4 → Admin → Property Details on `488680346` → **Move property** → destination account `348668675` "Spark Shark Analytics". Measurement ID stays the same; history preserved; no `build.py` change needed. After move, delete the redundant empty placeholder property `481482348` (already in Spark Shark Analytics — `G-JF1630186D` + `G-8SGD0GKF4F` streams, ~20 users, setup artifact). Re-verify the `sparkshark-seo-reader@...` SA still has Viewer post-move. Full plan in `SOURCE-OF-TRUTH.md` §11 "Pre-cutover: relocate property 488680346". **Sequence:** do this BEFORE the `build.py:153` GTM→gtag.js swap and BEFORE DNS flip. ~5 min.
...
- **Comprehensive backlink capture workstream** — in progress under `migration-evidence-pack/07-backlinks-and-citations/`. Session 1 of 2026-05-11 catalogued 27 unique referring domains across GSC + WebSearch + Bing API (Bing returned 0). Session 2 will scrape Ahrefs Webmaster Tools via Chrome MCP, pull GA4 referrals from Brock-owned properties (Flanco 480290314 + Spark Shark 488680346 once SA access is granted), and produce `master-backlinks-final.csv`. Read `migration-evidence-pack/07-backlinks-and-citations/SESSION-2-HANDOFF.md` to resume. Does NOT block cutover; the GSC view used in this runbook is sufficient for redirect-equity decisions.
...
- **Analytics-stack ownership reclaim audit (post-cutover).** Once `build.py:153` swap retires `GTM-W7V4RS7C` from the live site, audit whether any third party still has crawl / data access via the old container — e.g., shared Google Ads accounts, Search Console properties they own pointed at sparkshark.com. Not a cutover blocker. Full context in `SOURCE-OF-TRUTH.md` §11.
