...
- **Analytics ownership gap (2026-05-11 evening, backlink audit):** the unowned `GTM-W7V4RS7C` container's payload sets GA4 measurement ID `G-QK02QH3SWY`. That ID is NOT in any of Brock's three accessible GA4 properties (Flanco Electric `480290314`, Spark Shark `481482348`, Spark Shark `488680346`). Years of GA4 traffic data have flowed to a third-party-controlled property as a result. The `build.py:153` GTM→gtag.js swap below is the moment we lose `GTM-W7V4RS7C` from the live site — a feature, not a bug. Full inventory in `SOURCE-OF-TRUTH.md` §11 and `migration-evidence-pack/07-backlinks-and-citations/SESSION-2-HANDOFF.md` §10.
