...
|---|---|---|---|
| 1 | **Vercel custom-domain attachment is on the wrong team.** `sparkshark.com` apex is attached to team `spark-shark-electric` and assigned to project `spark-fsm` (the dispatcher), not the marketing site. `www.sparkshark.com` has no project assignment. The `sparkshark-com` project lives in a different team (`spark-shark-electric-2b2f3a3a`) with only `sscrm.tech` attached. If DNS flipped today, every sparkshark.com hit would land on the FSM dispatcher. | OPEN | Brock (~10 min in Vercel UI) |
| 2 | **Launch gate has 4 of 9 rows open.** Three need only a Status-cell promotion from existing evidence (Gate 4 GBP screenshots, Gate 7 Tracking IDs, Gate 9 Canonical NAP). Gate 8 needs a real new artifact — a ST scheduler smoke booking on the live alias. | OPEN | Brock (~90 min total) |
...
| 2 | **Author `tracking-ids.md`.** Document: GA4 measurement ID, GTM container ID (`GTM-TBCXCXGS` already in repo), Google Ads conversion ID/label if used, GTM-vs-gtag decision. | Gate #7 | Risk 6 (data corruption) | ~5 min |
...
> **Implicit prerequisite to all of the above:** fix the Vercel domain attachment (Blocker #1, §2). The 5-item queue above assumes the domain lives on the right project. If DNS flips while the apex is still bound to `spark-fsm`, none of these mitigations save the launch.
...
3. **Brock-owned queue scope.** The HTML runbook's 5-item Brock queue (§4 above) predates the DOM-01/02 discovery. The Vercel domain-attachment fix is required before that queue is meaningful, but is not numbered inside the queue itself. It appears as Blocker #1 in §2 and as an explicit prerequisite note under §4. Future revisions of the runbook should fold it in as a numbered step.
