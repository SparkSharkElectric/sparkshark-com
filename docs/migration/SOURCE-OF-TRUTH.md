...
- **Re-verification audit (independent 20:13 UTC vantage, 2026-05-11):** `audits/migration-readiness/2026-05-11-2013utc-reverification.md` — second-vantage curl sweep ~4h after the canonical 16:24 UTC run; verdict unchanged. Three checks UNVERIFIED in the re-run (DOM-01/02 sandbox lacked Vercel CLI; SEC-01 no `.gitleaks.toml`; GBP screenshots not OCR'd).
- **Site-hygiene audit (2026-05-11, technical + plain-English):** `audits/site-hygiene/2026-05-11-technical.md` and `audits/site-hygiene/2026-05-11-plain-english.md` — score 76/100. HIGH items: favicon is the full logo scaled down (will render fuzzy at 32×32 and get cached aggressively at first crawl); two competing GTM containers (already resolved by §11 option-3 decision). MEDIUM: duplicate FAQPage schema on every service page — touches `build.py` schema generators, ADR-required.
- **Coalition Technologies engagement detail:** `docs/migration/coalition-findings.md` — Oct–Dec 2025 engagement, link-building (0–5 estimated), 5 copy batches shipped to WP, technical SEO changes, tracking installs. Source of `GTM-W7V4RS7C` and `G-QK02QH3SWY` (per §11). Cancellation Dec 16 2025; services ended Jan 15 2026.
- **Online presence inventory:** `docs/migration/online-presence-inventory.md` — ~25 external surfaces (GBP, citations, social, reviews, tracking, schema, widgets, DNS). 14 gaps catalogued (Apple Maps decision, Facebook page rename, Yelp URL capture, BBB Flanco profile audit, LinkedIn page, disavow file, `flancoelectricok.com` domain status, SEOPress license cancellation, etc.).
- **Prior SEO evidence umbrella:** `docs/migration/prior-seo-evidence.md` — cross-vendor index across three eras (Strategy Zoo current, Coalition cancelled, Flanco-era BigThinkers pending — Brock requested a dedicated session for the Flanco scrape).
...
> **Status 2026-05-12 — Phases 1 & 2 done; the five artifacts are now committed (no longer untracked).** **Phase 1 (triage):** no contradictions with §1–§12; two watch-items — the `GT-NGS794C2` ↔ `G-QK02QH3SWY` measurement-ID equivalence still needs verifying (per §11), and two disavow workstreams are converging (the Ahrefs-surfaced 83-domain file + the pending Flanco-era BigThinkers/rachelhall PBN set) — both captured in `docs/migration/action-items.md`. **Commit-destination decision:** the three `docs/migration/*.md` evidence files stay in `docs/migration/` for this cycle (the canonical docs reference them at those paths; the Flanco session writes `flanco-findings.md` there too) — revisit a `docs/migration/research/` reorg when `flanco-findings.md` lands, moving all four together and updating the reference paths in §9 here + `cutover-runbook.md` §9 in the same PR. Audits stay in `audits/`. **Phase 2 (action tracker):** → **`docs/migration/action-items.md`** — the single ranked list (A = blocks DNS flip, B = blocks post-flip monitoring, C = optional/post-launch), de-duplicated against §4/§5. **Phase 3 (PR sequence):** recommended at the bottom of `action-items.md` (recommend only, not executed). **Phase 4 (out-of-scope):** flagged in `action-items.md`. Original brief kept below for provenance.

After commit `12d2eff` shipped the option-3 tracking decision and the GSC-DNS-TXT continuity confirmation, five additional artifacts were authored 2026-05-11 and remain untracked in the working tree (paths in §9: re-verification audit, site-hygiene audit folder, `coalition-findings.md`, `online-presence-inventory.md`, `prior-seo-evidence.md`). A session picking up this work should run the four phases below in order.

**Phase 1 — triage and commit-destination plan (read-only).** Read all 5 artifacts. Confirm none duplicate or contradict §1–§12. Recommend whether the three `docs/migration/*.md` evidence files stay where they are or move to a new `docs/migration/research/` subfolder to keep the authority chain (`launch-gate.md` + this SoT + `cutover-runbook.md`) uncluttered (precedent: §10 already suggests `sparkshark-com/docs/research/` for the two large reference research files). Audits stay in `audits/`.

**Phase 2 — single action-item tracker.** Pull every Action / Gap / Pending / Verify / TODO out of all 5 artifacts. Cross-reference against §4 (Brock-owned queue, 7 items including the 2026-05-11 evening additions) and §5 (Claude-owned queue, 6 items); flag duplicates. Output a single ranked file ordered by (a) blocks DNS flip, (b) blocks post-flip monitoring, (c) optional cleanup. Do not scatter findings across the 5 source artifacts.

**Phase 3 — sequence recommendation (recommend, do not execute).** Propose the next 2–4 PRs in priority order. Likely candidates after reading: (i) `build.py:153` option-3 patch — already documented in §4 row 2; closes the unowned `GTM-W7V4RS7C` payload as a side-effect, (ii) GA4 property `488680346` move to Spark Shark Analytics per §4 row 7 + §11, (iii) favicon dedicated-set fix per site-hygiene HIGH #1 (cheapest fix; closes the most-cached identity asset before Google's first crawl on the new host), (iv) typed-out DNS rollback values to close Blocker #3 in §2. One scoped PR per change per the "Surgical Patches Stay Single-File" rule.

**Phase 4 — explicit out-of-scope flags.** Flanco-era SEO scrape (Brock requested a dedicated session). Coalition asset transfer-out — Semrush + Ads MCC (Brock-owned). Strategy Zoo relationship decision — 3 payment failures Apr 23–25 noted (Brock-owned business call). FAQPage duplicate-schema fix from site-hygiene MEDIUM (touches protected schema generators — ADR required, not a launch blocker).

**Guardrails (from `~/.claude/CLAUDE.md` + repo `CLAUDE.md`):** public brand "Spark Shark Electric" with a space; Brock is Founder, CEO/operator — never owner / partner / equity / legal owner; no autonomous commits or pushes in `sparkshark-com` (push = deploy: Vercel + GH Pages preview rebuild on every push to `main`); protected surfaces in `build.py` (BRAND dict, schema generators, `ST_API_KEY`, `ST_SCHEDULER_ID`); never re-add `CNAME` at repo root (Vercel handles the custom domain at the project level); PR-authority procedure (per migration-auditor skill) applies for any change that ships.
...
Begin by reading the 5 artifacts in §9. After Phase 1 (triage + commit-destination plan), pause and confirm with Brock before staging anything.
