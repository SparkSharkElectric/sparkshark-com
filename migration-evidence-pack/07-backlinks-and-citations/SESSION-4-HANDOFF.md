...
- ~~[Brock] Verify the GA4 "candidate backlink" mentions~~ — **DONE 2026-05-12** (see `BACKLINK-SUMMARY.md` §1.7). None is an on-page editorial backlink: `chatgpt.com` + `claude.ai` are real LLM citations (a GEO signal, not a link to fix), `hometalk.com` has no findable on-page link, `marketspacesales.com` is a dead domain / likely referrer-spam. All 4 → P3. Tier counts shifted P2 20→17 / P3 26→29 (total still 145); the useful-backlink count is unchanged. *(GEO note: both ChatGPT and Claude have served `sparkshark.com` links in user conversations — worth tracking as an AI-visibility signal.)*
