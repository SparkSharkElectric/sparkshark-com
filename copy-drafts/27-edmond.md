# COPY DRAFT — /locations-we-serve/edmond/
# File: copy-drafts/27-edmond.md

<!-- INSTRUCTIONS -->
<!-- Edit anything below. Keep the section headers (===) intact so I can re-merge cleanly. -->
<!-- Lines starting with <!-- are comments and won't appear on the site. -->
<!-- Bullets become <li>. **bold** stays bold. Links: word [https://url] format. -->
<!-- When done, hand me the edited file(s) and I'll regenerate the site. -->

<!-- NOTE: City angle = EV chargers, newer homes, smart home, panels. -->

=== TITLE (browser tab — 50–60 chars ideal, max 60) ===
Electrician Edmond OK | EV Chargers & Smart Home | Spark Shark

=== META DESCRIPTION (Google snippet — 140–160 chars) ===
Licensed residential electrician serving Edmond, OK: EV charger installs, smart-home wiring, panel upgrades, generators. Spark Shark Electric. (405) 436-4776.

=== H1 (main page heading — visible on page) ===
Residential Electrician in Edmond, OK

=== HERO SUBTITLE (1–2 sentences under H1) ===
Licensed residential electrical service for Edmond: Level 2 EV chargers, smart-home wiring, panel upgrades, and 24/7 emergency dispatch.

=== INTRO PARAGRAPH (lede paragraph at top of body) ===
Spark Shark Electric is a licensed residential electrician serving Edmond, Oklahoma. Headquartered in Moore at [VERIFY: canonical business address], we work in Edmond every week — newer homes adding Level 2 EV chargers, smart-home retrofits, recessed lighting upgrades, and the occasional panel change-out for a remodel or hot-tub addition. Every job is flat-rate and written before work begins. Call (405) 436-4776.

=== SECTION: What we do for Edmond homeowners ===
- **Level 2 EV charger installation**: Tesla Wall Connector, ChargePoint, Wallbox, Ford Charge Station Pro, and other UL-listed brands.
- **Smart-home wiring**: Lutron Caseta, Lutron RA, Leviton Decora Smart, structured cable, in-wall A/V.
- **Electrical panel upgrades**: when a renovation, EV charger, or hot tub demands more service capacity.
- **Whole-home Generac generators**: for backup against Oklahoma storms.
- **Indoor lighting upgrades**: recessed cans, under-cabinet, dimmers compatible with LED loads.
- **24/7 emergency electrical service**: burning smells, hot panels, sparking outlets.
- **Safety inspections**: pre-purchase or pre-renovation.

=== SECTION: Why we cover Edmond ===
- **Newer homes, modern loads.** Edmond construction trends younger than much of the metro — which means EV chargers, smart switches, and outdoor lighting are common upgrade requests rather than panel-replacement jobs.
- **Flat-rate, written pricing.** No trip charge, no diagnostic fee. The number we quote is the number you pay.
- **Residential-only.** We don't split focus between commercial and homes — every job is a residence.

=== FAQ ===
Q: Can you install any EV charger I buy?
A: Yes. Any UL-listed Level 2 charger. We don't lock you into a brand. [VERIFY: EV charger brands/certifications].

Q: Do I need to upgrade my panel for an EV charger?
A: Often no. Most newer Edmond homes have 200A panels with capacity for a 50A EV circuit. We verify on the site visit.

Q: Are you licensed to work in Edmond?
A: Yes. [VERIFY: Oklahoma CIB license #00163603]. Licensed and insured.

Q: Do you handle Edmond permits and inspections?
A: Yes. We pull the permit and schedule the inspection as part of every flat-rate quote where a permit applies. [VERIFY: City of Edmond permit/inspection language].

<!-- INTERNAL LINK SUGGESTIONS:
     Link to EV Charger Installation, Smart Home Installation, Indoor Lighting, Electrical Panels,
     Neighbor cities: Yukon and Oklahoma City.
     About, Reviews, Contact. -->

<!-- SCHEMA RECOMMENDATION:
     Service with areaServed: { @type: "City", name: "Edmond" }, FAQPage, BreadcrumbList. -->
