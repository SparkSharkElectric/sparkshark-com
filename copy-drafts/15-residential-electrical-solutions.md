# COPY DRAFT — /residential-electrical-solutions/
# File: copy-drafts/15-residential-electrical-solutions.md

<!-- INSTRUCTIONS -->
<!-- Edit anything below. Keep the section headers (===) intact so I can re-merge cleanly. -->
<!-- Lines starting with <!-- are comments and won't appear on the site. -->
<!-- Bullets become <li>. **bold** stays bold. Links: word [https://url] format. -->
<!-- When done, hand me the edited file(s) and I'll regenerate the site. -->

=== TITLE (browser tab — 50–60 chars ideal, max 60) ===
Residential Electrician Moore & OKC | Spark Shark Electric

=== META DESCRIPTION (Google snippet — 140–160 chars) ===
Full-service residential electrician in Moore and Oklahoma City. Panels, generators, repair, installation, lighting, EV chargers. Flat-rate. (405) 436-4776.

=== H1 (main page heading — visible on page) ===
Residential Electrical Solutions in Moore and the Oklahoma City Metro

=== HERO SUBTITLE (1–2 sentences under H1) ===
Everything we do, in one place — built around homes, homeowners, and the electrical needs of OKC-metro neighborhoods.

=== INTRO PARAGRAPH (lede paragraph at top of body) ===
Spark Shark Electric in Moore, Oklahoma is a residential-only electrical contractor serving the OKC metro. That focus matters: we know homes, we know the wiring patterns of older OKC and Moore neighborhoods and the modern panels in newer Edmond and Yukon subdivisions, and we don't try to cross-sell you commercial-style work you don't need. Every job is flat-rate and written before work begins.

=== SECTION: Categories of work we do ===
- **Panel upgrades** — service replacement, sub-panels, code upgrades.
- **Generator installation** — whole-home Generac standby with automatic transfer.
- **Emergency electrical service** — 24/7 dispatch.
- **Repair** — flickering, dead outlets, breaker trips, GFCI/AFCI faults.
- **Installation** — new circuits, rewires, smart-home, EV chargers.
- **Safety inspections** — pre-purchase, pre-renovation, insurance-required.
- **Smoke + CO detectors** — hardwired interconnected systems.
- **Switches and outlets** — replacements, additions, GFCI/AFCI.
- **Outdoor lighting** — landscape, security, architectural.
- **Indoor lighting** — recessed, under-cabinet, pendants, dimmers.
- **Ceiling fans** — replacements and new installs.
- **EV chargers** — Level 2 home stations.
- **Smart home** — smart switches, structured cable, A/V.

=== FAQ ===
Q: Do you do commercial electrical work?
A: No. Spark Shark Electric is a residential-only electrical contractor. We focus exclusively on homes.

Q: Are you licensed?
A: Yes. [VERIFY: Oklahoma Electrical License #00163603 active and in good standing per Oklahoma CIB lookup]. Licensed and insured.

Q: Do you offer free estimates?
A: Yes. Every estimate is free and written. Flat-rate pricing — no surprises.

Q: How fast can you get out?
A: Same-day where available for emergencies. Next-day to within a few days for scheduled work, depending on backlog. We'll tell you the realistic schedule when you call.

<!-- INTERNAL LINK SUGGESTIONS:
     Link to every individual service page (Panels, Generators, Emergency, Repair, Installation,
     Inspections, Smoke Detectors, Switches & Outlets, Outdoor Lighting, Indoor Lighting,
     Ceiling Fans, EV Charger, Smart Home), Reviews, About, Contact. -->

<!-- SCHEMA RECOMMENDATION:
     Service (serviceType: "Residential Electrical Services"), FAQPage, BreadcrumbList. -->
