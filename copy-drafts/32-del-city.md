# COPY DRAFT — /locations-we-serve/del-city/
# File: copy-drafts/32-del-city.md

<!-- INSTRUCTIONS -->
<!-- Edit anything below. Keep the section headers (===) intact so I can re-merge cleanly. -->
<!-- Lines starting with <!-- are comments and won't appear on the site. -->
<!-- Bullets become <li>. **bold** stays bold. Links: word [https://url] format. -->
<!-- When done, hand me the edited file(s) and I'll regenerate the site. -->

<!-- NOTE: City angle = repairs, panels, emergency electrical, older housing stock. -->

=== TITLE (browser tab — 50–60 chars ideal, max 60) ===
Electrician Del City OK | Repair & Panels | Spark Shark

=== META DESCRIPTION (Google snippet — 140–160 chars) ===
Licensed residential electrician serving Del City, OK — repair, panel upgrades, 24/7 emergency, safety inspections. Spark Shark Electric. (405) 436-4776.

=== H1 (main page heading — visible on page) ===
Residential Electrician in Del City, OK

=== HERO SUBTITLE (1–2 sentences under H1) ===
Licensed residential electrical service for Del City — repair, panel upgrades, 24/7 emergency dispatch, and free safety inspections with any paid call.

=== INTRO PARAGRAPH (lede paragraph at top of body) ===
Spark Shark Electric is a licensed residential electrician serving Del City, Oklahoma. Headquartered in Moore at [VERIFY: canonical business address], we work in Del City's older neighborhoods every week — repair calls for flickering lights and breaker trips, panel upgrades for homes still on undersized service or older brand panels, and 24/7 emergency dispatch when something goes wrong overnight or on a weekend. Flat-rate, written pricing on every job. Call (405) 436-4776.

=== SECTION: What we do for Del City homeowners ===
- **Electrical repair** — flickering lights, dead outlets, breaker trips, GFCI/AFCI faults, partial power loss.
- **Electrical panel upgrades** — service replacement, 100A→200A, sub-panels, Federal Pacific / Zinsco swaps.
- **24/7 emergency electrical service** — burning smells, hot panels, sparking outlets.
- **Safety inspections** — free with any paid service call.
- **Whole-home Generac generators** — for Oklahoma storms.
- **Installation** — new circuits, smart-home, EV chargers, lighting.

=== SECTION: Why we cover Del City ===
- **Older housing stock.** Del City has a strong base of mid-century homes that often need updated panels, GFCI/AFCI compliance, and standard repair work.
- **Same flat-rate as the rest of the metro.** No zone surcharges, no separate dispatch fees.
- **24/7 dispatch matters.** Older wiring sometimes fails after hours. We answer the phone live.

=== SECTION: Del City FAQ ===
Q: Are you licensed to work in Del City?
A: Yes. [VERIFY: Oklahoma CIB license #00163603]. Licensed and insured.

Q: Can you handle a Federal Pacific panel replacement?
A: Yes — we replace Federal Pacific Stab-Lok, Zinsco/Sylvania, and other older or recalled panel brands every week.

Q: Do you handle Del City permits and inspections?
A: Yes — we pull the permit and schedule the inspection as part of every flat-rate quote where a permit applies. [VERIFY: City of Del City permit/inspection language].

<!-- INTERNAL LINK SUGGESTIONS:
     Link to Electrical Repair, Electrical Panels, Emergency Electrician, Electrical Inspections,
     Neighbor cities: Midwest City and Oklahoma City.
     About, Reviews, Contact. -->

<!-- SCHEMA RECOMMENDATION:
     Service with areaServed: { @type: "City", name: "Del City" }, FAQPage, BreadcrumbList. -->
