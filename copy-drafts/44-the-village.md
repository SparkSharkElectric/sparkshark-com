# COPY DRAFT — /locations-we-serve/the-village/
# File: copy-drafts/44-the-village.md

<!-- INSTRUCTIONS -->
<!-- Edit anything below. Keep the section headers (===) intact so I can re-merge cleanly. -->
<!-- Lines starting with <!-- are comments and won't appear on the site. -->
<!-- Bullets become <li>. **bold** stays bold. Links: word [https://url] format. -->
<!-- When done, hand me the edited file(s) and I'll regenerate the site. -->

<!-- NOTE: City angle = small bedroom community NW OKC, post-WWII ranch homes 1955-1965, original 60-100A panels, aluminum-branch wiring possible, two-prong outlets, smoke detector retrofits. -->

=== TITLE (browser tab — 50–60 chars ideal, max 60) ===
Electrician The Village OK | Panels & Rewires | Spark Shark

=== META DESCRIPTION (Google snippet — 140–160 chars) ===
Licensed residential electrician serving The Village, OK: panel upgrades, aluminum-branch remediation, outlet and smoke-detector retrofits, 24/7 emergency. Spark Shark.

=== H1 (main page heading — visible on page) ===
Residential Electrician in The Village, OK

=== HERO SUBTITLE (1–2 sentences under H1) ===
Licensed residential electrical service for The Village: panel upgrades, two-prong outlet and aluminum-branch remediation, smoke-detector retrofits, EV chargers, and 24/7 emergency dispatch.

=== INTRO PARAGRAPH (lede paragraph at top of body) ===
Spark Shark Electric is a licensed residential electrician serving The Village, Oklahoma. Headquartered in Moore, about 30 minutes via I-44 North, we cover The Village's mid-century housing stock: post-WWII ranch homes built between 1955 and 1965, where the original 60–100A panels, two-prong outlets, and the occasional aluminum branch circuit are still in service. Panel upgrades, outlet replacements, and modern safety retrofits are the common asks here. Same flat-rate pricing as the rest of the OKC metro. Call (405) 436-4776.

=== SECTION: What we do for The Village homeowners ===
- **Electrical panel upgrades**: 60A or 100A→200A service replacement, sub-panels, Federal Pacific and Zinsco swaps.
- **Aluminum branch wiring remediation**: COPALUM crimps or AlumiConn pigtails at every device, code-compliant.
- **Two-prong outlet replacement**: GFCI-protected three-prong upgrades where ground isn't available; full rewiring where it is.
- **Smoke and CO detector retrofits**: hard-wired, interconnected, 10-year sealed-battery backup.
- **Level 2 EV charger installation**: any UL-listed brand.
- **24/7 emergency electrical service.**
- **Safety inspections**: free with any paid service call.

=== SECTION: Why we cover The Village ===
- **Original electrical from the Eisenhower era.** A lot of Village homes still run on the same panel they were built with. 60-amp service, fuse boxes in the older builds, two-prong outlets throughout. Most homeowners we meet here want to know what's safe to leave and what should be replaced — we tell them straight.
- **Aluminum branch wiring shows up.** Some 1960s additions used aluminum on 15A and 20A branch circuits. It's not automatically a remove-it situation — properly pigtailed with COPALUM or AlumiConn, it's safe. We do the work to code and document it for insurance.
- **Quick run from Moore HQ.** About 30 minutes via I-44. Same-day for emergencies.

=== FAQ ===
Q: Are you licensed to work in The Village?
A: Yes. [VERIFY: Oklahoma CIB license #00163603]. Licensed and insured.

Q: Do you handle The Village permits and inspections?
A: Yes. Every job that needs a permit gets one as part of our flat rate. [VERIFY: City of The Village permit/inspection language and inspector cadence.]

Q: My home has two-prong outlets and no ground wire. What can you actually do?
A: Three options, in order of cost: (1) GFCI-protect the upstream outlet on each circuit and replace downstream outlets with three-prong marked "GFCI Protected, No Equipment Ground" — code-legal, lowest cost, no new wire pulled. (2) Run new grounded cable to specific high-priority outlets (TV, computer, kitchen). (3) Whole-home or partial rewire. We quote all three and let you pick.

<!-- INTERNAL LINK SUGGESTIONS:
     Link to Electrical Panels, Switches and Outlets, Electrical Repair, Smoke Detectors.
     Neighbor cities: Nichols Hills (south), Warr Acres (west), Oklahoma City (south).
     About, Reviews, Contact. -->

<!-- SCHEMA RECOMMENDATION:
     Service with areaServed: { @type: "City", name: "The Village" }, FAQPage, BreadcrumbList. -->
